...
export default function ScreenshotTransformControls({target, settings, style, selected, onSelect}: Props) {
    ...
    <div
      ...
      onPointerMove={(event) => {
        ...
        if (g.mode === "move") {
          ...
          update({imageScale: Math.max(50, Math.min(150, g.initial.imageScale * ratio))});
        } else {
      ...
      className="absolute z-20 touch-none select-none focus:outline-none"
      style={{
        ...
        {(["top-left", "top-right", "bottom-left", "bottom-right"] as const).map((corner) => (
          <button
            ...
            onKeyDown={(event) => {
              ...
              update({imageScale: Math.max(50, Math.min(150, settings.imageScale + direction * (event.shiftKey ? 10 : 1)))});
            }}
